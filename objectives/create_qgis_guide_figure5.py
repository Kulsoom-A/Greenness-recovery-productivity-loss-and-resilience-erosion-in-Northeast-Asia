...
def main() -> None:
    ...
    spei, raster_extent = read_raster(OBJ5 / "data" / "processed_tif" / "mean_event_spei12.tif")
    tmax, _ = read_raster(OBJ5 / "data" / "processed_tif" / "mean_event_tmax_z.tif")
    vulnerability, _ = read_raster(OBJ5 / "data" / "processed_tif" / "rf_predicted_vulnerability_index.tif")
    probability, _ = read_raster(OBJ5 / "data" / "processed_tif" / "rf_severe_hotspot_probability.tif")

    plot_extent = valid_extent([spei, tmax, vulnerability, probability], raster_extent)
    boundary = gpd.read_file(SHP).to_crs("EPSG:4326")

    spei_lim = np.nanpercentile(np.abs(spei), 98)
    tmax_lim = np.nanpercentile(np.abs(tmax), 98)
    ...
    add_panel(
        ...
        "BrBG",
        "SPEI-12",
        norm=TwoSlopeNorm(vcenter=0, vmin=-spei_lim, vmax=spei_lim),
    )
    add_panel(
        axes[1],
        tmax,
        ...
        boundary,
        "B",
        "Mean event Tmax z-score",
        "coolwarm",
        "Tmax z-score",
        norm=TwoSlopeNorm(vcenter=0, vmin=-tmax_lim, vmax=tmax_lim),
    )
    add_panel(
        axes[2],
        vulnerability,
        ...
        boundary,
        "C",
        "RF predicted vulnerability index",
        "magma",
        "index",
        vmin=0,
        vmax=1,
    ...
    out_png = OUT / "qgis_guide_figure5_spatial_controls_predicted_vulnerability.png"
    out_pdf = OUT / "qgis_guide_figure5_spatial_controls_predicted_vulnerability.pdf"
