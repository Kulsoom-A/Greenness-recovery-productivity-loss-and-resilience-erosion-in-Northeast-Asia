...
def add_panel(ax, arr, raster_extent, plot_extent, boundary, panel, title, cmap, legend_label, vmin=None, vmax=None, ticks=None):
...
def main() -> None:
    ...
    base = OBJ99 / "data" / "processed_tif"
    ndvi, raster_extent = read_raster(base / "NDVI_hidden_mismatch_duration_t1_t4.tif")
    no_freq, _ = read_raster(base / "loo_severe_without_event_frequency.tif")
    no_hidden, _ = read_raster(base / "loo_severe_without_hidden_mismatch_duration.tif")
    no_gpp_loss, _ = read_raster(base / "loo_severe_without_gpp_cumulative_loss.tif")

    ndvi = np.where(ndvi > 0, ndvi, np.nan)
    no_freq = np.where(no_freq == 1, no_freq, np.nan)
    no_hidden = np.where(no_hidden == 1, no_hidden, np.nan)
    no_gpp_loss = np.where(no_gpp_loss == 1, no_gpp_loss, np.nan)

    plot_extent = valid_extent([ndvi, no_freq, no_hidden, no_gpp_loss], raster_extent)
    ...
    add_panel(axes[0], ndvi, raster_extent, plot_extent, boundary, "A", "NDVI-GPP hidden mismatch duration", "magma", "years", vmin=1, vmax=4, ticks=[1, 2, 3, 4])
    add_panel(axes[1], no_freq, raster_extent, plot_extent, boundary, "B", "Severe hotspot without event frequency", "Reds", "1 = severe hotspot", vmin=0, vmax=1, ticks=[0, 1])
    add_panel(axes[2], no_hidden, raster_extent, plot_extent, boundary, "C", "Severe hotspot without hidden mismatch duration", "Reds", "1 = severe hotspot", vmin=0, vmax=1, ticks=[0, 1])
    add_panel(axes[3], no_gpp_loss, raster_extent, plot_extent, boundary, "D", "Severe hotspot without GPP cumulative loss", "Reds", "1 = severe hotspot", vmin=0, vmax=1, ticks=[0, 1])
    fig.subplots_adjust(left=0.055, right=0.985, top=0.955, bottom=0.07, wspace=0.12, hspace=0.34)
    out_png = OUT / "qgis_guide_supp_robustness_spatial_checks.png"
    out_pdf = OUT / "qgis_guide_supp_robustness_spatial_checks.pdf"
